...
function setupHandlers(){
  ...
  if (dl) dl.addEventListener('click', ()=>{
    ...
    const w = container.clientWidth || 900; const h = Math.max(240, Math.floor((vms.length+1)*40));
...
function renderTopology(){ const container=document.getElementById('topology'); if(!container) return; container.innerHTML=''; const width = container.clientWidth || 800; const height = Math.max(240, Math.floor((vms.length+1)*40)); const svgNS='http://www.w3.org/2000/svg'; const svg=document.createElementNS(svgNS,'svg'); svg.setAttribute('viewBox',`0 0 ${width} ${height}`); svg.setAttribute('preserveAspectRatio','xMidYMid meet'); svg.setAttribute('xmlns',svgNS);
  const masters = vms.filter(n=>n.role==='master'); const workers = vms.filter(n=>n.role==='worker'); const topY=60; const botY=height-60; const mCount=Math.max(1,masters.length); const wCount=Math.max(0,workers.length);
